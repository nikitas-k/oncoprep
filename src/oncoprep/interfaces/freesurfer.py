...
#
"""Nipype FreeSurfer interface customizations and wrappers."""
...
class _ReconAllInputSpec(fs.preprocess.ReconAllInputSpec):
    """Custom input specification for ReconAll with additional directive options."""
...
class ReconAll(fs.ReconAll):
    """
    Custom FreeSurfer recon-all interface with resume capability.

    This interface extends Nipype's ReconAll to provide intelligent resumption
    of recon-all pipelines by checking which steps have already completed and
    skipping them on subsequent runs. It also provides better control over
    specific processing steps.

    Extends
    -------
    nipype.interfaces.freesurfer.ReconAll
    ...
    def cmdline(self):
        """
        Generate command-line string with resume intelligence.

        Returns
        -------
        str
            Command-line string for recon-all execution
    ...
    def _format_arg(self, name, trait_spec, value):
        """
        Format argument, handling hemisphere specification properly.

        Parameters
        ----------
        name : str
            Argument name
        trait_spec : traits.TraitType
            Trait specification
        value : any
            Argument value
...
class _MRIsConvertDataInputSpec(fs.utils.MRIsConvertInputSpec):
    """Input specification for MRIsConvertData with data overlay support."""
...
class MRIsConvertData(fs.utils.MRIsConvert):
    ...
    passed a file from the subject's surf/ directory. This simplifies working
    with overlay data by inferring the surface file from the data file.
    ...
    def _gen_filename(self, name):
        """
        Generate missing filenames intelligently.
...
class MakeMidthicknessInputSpec(nwfs._MakeMidthicknessInputSpec, fs.base.FSTraitedSpecOpenMP):
    """Input specification for MakeMidthickness with OpenMP support."""
...
class MakeMidthickness(nwfs.MakeMidthickness, fs.base.FSCommandOpenMP):
    """
    Patched MakeMidthickness interface for robust midthickness surface creation.
    ...
    nipype.interfaces.freesurfer.base.FSCommandOpenMP

    Examples
    --------
    >>> from oncoprep.interfaces.freesurfer import MakeMidthickness
    ...
    def _format_arg(self, name, trait_spec, value):
        """
        Format argument with hemisphere label handling.
    ...
    def _num_threads_update(self):
        """Update OpenMP thread count for optimal performance."""
        if self.inputs.num_threads:
            self.inputs.environ.update({'OMP_NUM_THREADS': str(self.inputs.num_threads * 3 // 2)})


__all__ = ['ReconAll', 'MRIsConvertData', 'MakeMidthickness']
